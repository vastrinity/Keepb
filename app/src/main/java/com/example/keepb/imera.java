...
public class imera extends AppCompatActivity implements CompoundButton.OnCheckedChangeListener, RatingBar.OnRatingBarChangeListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        telosimeras=findViewById(R.id.telosmera);
        ...
        telosimeras.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
             savemera();
            }
    ...
    public void saveswitches(){
        ...
        if(simera.equals(""))
        {
            int lastday = mydbimera.getImeraLast();
            int today = lastday+1;
            simera = String.valueOf(today);
        ...
        if (insertImera=true)
            Toast.makeText(this," καταχωρηθηκε",Toast.LENGTH_LONG).show();
        else
            Toast.makeText(this," δεν καταχωρηθηκε",Toast.LENGTH_LONG).show();
    ...
    public void savemera(){
        saveswitches();


        int sumgeymata = (ibtv.isChecked()?1:0)+(ibwv.isChecked()?1:0)+(idtv.isChecked()?1:0)+(idwv.isChecked()?1:0)+(imtv.isChecked()?1:0)+(imwv.isChecked()?1:0)+
                (iatv.isChecked()?1:0)+(iawv.isChecked()?1:0)+(iltv.isChecked()?1:0)+(ilwv.isChecked()?1:0);
        int sumo =(icocav.isChecked()?1:0)+(ialcoolv.isChecked()?1:0);


        if(sumgeymata==10 && iwaterv.getRating()>=2 ){
